...
async def isincache(sreq: body_model.scenario_req):
    res = CACHE.get(sreq.user_id, sreq.id)
...
async def getfromcache(sreq: body_model.scenario_req):
    res = CACHE.get(sreq.user_id, sreq.id).copy()
    res.pop("graph")
    return JSONResponse(
        status_code=200,
        content=res
...
async def deletefromcache(sreq: body_model.scenario_req):
    res = CACHE.invalidate(sreq.user_id, sreq.id)
    return JSONResponse(
        status_code=200,
        content=res
    )
...
async def load(sreq: body_model.scenario_req, g: GraphDep):
    res = CACHE.put(sreq.user_id, sreq.id, g, sreq.closed_segments, sreq.alfa, sreq.manual_weights)
    return JSONResponse(
        status_code=200,
        content=res
    )