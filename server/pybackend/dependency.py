...
def get_scenario_graph(
    ...
    scen = CACHE.get(sreq.uid, sreq.id)
    if(scen):
        g=scen["graph"]
        CACHE.update(sreq.uid, sreq.id, {"last_access": time.time()})
        ...
        CACHE.put(sreq.uid, sreq.id, g, sreq.closed_segments, sreq.alfa, sreq.manual_weights)
    return g
