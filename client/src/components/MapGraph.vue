...
import { GraphService } from '../services/graphService'; // 🔥 Importiamo il nuovo servizio
...
export default {
  name: 'MapGraph',
  props: { focusEdgeId: String, closedEdges: { type: Array, default: () => [] }, startPoint: Object, endPoint: Object, cursor: String, routingPath: { type: Array, default: () => [] } },
  ...
  setup(props, { emit }) {
    ...
    const initMap = () => {
      ...
      map.value = L.map(mapContainer.value, { zoomControl: false, preferCanvas: true, maxBounds: TRENTO_BOUNDS, minZoom: 12 }).setView([46.0665, 11.1216], 15);
    ...
    const loadGraph = async () => {
      ...
      loading.value = true;
      try {
        // 🔥 Uso pulitissimo del Servizio: nessuna matematica nel componente!
        const { geojsonData, processedEdges } = await GraphService.fetchAndProcessGraph();
        
        const geojson = L.geoJSON(geojsonData, {
          coordsToLatLng: GraphService.convertCoords,
          ...
          onEachFeature: (feature, layer) => {
            ...
            layer.on('click', (e) => {
              ...
              emit('select-edge', { ...edgeData, latlng: e.latlng, isClosed: !!feature.properties.isClosed, isMulti: e.originalEvent.ctrlKey });
            });
          }
        });
        ...
        dssStore.allEdges = processedEdges; 
