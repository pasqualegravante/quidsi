...
export default {
  ...
  methods: {
    handleKeydown(e) {
      ...
      // Shortcuts da tastiera per Undo/Redo
    ...
    handleLoadScenario(scenario) {
      ...
      if (scenario.closed_edges) {
        // Pulisce l'history quando si carica un intero scenario nuovo per evitare bug
...
/* NUOVI TASTI ICONA PER UNDO REDO */
...
.preview-mode { background: #f1f5f9 !important; overflow: hidden !important; }
.preview-mode .dss-header, .preview-mode .routing-widget, .preview-mode .sidebar, .preview-mode .vehicle-selector, .preview-mode .btn-system, .preview-mode .btn-reset, .preview-mode .close-fs { display: none !important; }
.preview-mode .print-header-section, .preview-mode .print-footer-section { display: none !important; }
.preview-mode .dss-viewport, .preview-mode .map-wrapper, .preview-mode #map { height: 12cm !important; width: 100% !important; position: relative !important; margin-top: auto; margin-bottom: auto; }
...
@media print {
  ...
  .dss-header, .routing-widget, .closures-widget, .vehicle-selector, .fs-overlay, .toast-container, .leaflet-control-container, .global-overlay { display: none !important; }
  .dss-main-container { display: block !important; height: auto !important; }
  .dss-main-container.preview-mode .print-header-section, .dss-main-container.preview-mode .print-footer-section, .print-header-section, .print-footer-section { display: block !important; width: 100%; font-family: 'Inter', sans-serif; color: black; }
